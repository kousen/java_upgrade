...
public class LazyStreams {
    ...
    public static int multByTwo(int n) {
        System.out.printf("Inside multByTwo with arg %d (thread %s)%n", n, Thread.currentThread().getName());
        return n * 2;
    }

    public static boolean modByThree(int n) {
        System.out.printf("Inside modByThree with arg %d (thread %s)%n", n, Thread.currentThread().getName());
        return n % 3 == 0;
    }
